...
const chatContainer = () => {
  ...
  const { authUser, peer, peerId, callerName, socket, getPeerId, friendPeerId } = useAuthStore()
  const {
    streamSet,
    getStream,
    setStreamMode,
      ...
      {incomingCall && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 shadow-lg text-center">
              <h2 className="text-xl font-bold mb-2">Incoming Call from {incomingCall?.callerName || callerName || "User"}</h2>
                ...
                <button
                  onClick={() => {
                    socket.emit("join-room", {
                      ...
                      emailId: authUser?.email || authUser?.fullName || "User"
                    });
